...
test("Is logging", async function(t) {
  return await check(INVALID_TOKEN).catch(function() {
...
test.serial("Invalid token", async function(t) {
  ...
  return await check(INVALID_TOKEN).then(function() {
...
test.serial("No valid tokens", async function(t) {
  ...
  return await emptyCheck(TOKEN_1).then(function() {
    t.fail("Should have failed");
  }).catch(function(error) {
    t.is(error.message, "Invalid token", "Invalid token message");
  });
...
test.serial("Valid token", async function(t) {
  ...
  return await check(TOKEN_1).then(function() {
    t.pass("Token valid");
  }).catch(function(error) {
...
test("Resolve with decoded token", async function(t) {
  return await check(TOKEN_1).then(function(decodedToken) {
